"""Collection retrieval and mapping helpers for OAI-PMH."""
...
GeneralInfoPrefetch = Prefetch(
    "general_info",
    queryset=CollectionGeneralInfo.objects.select_related("location").prefetch_related("object_languages"),
)
...
AdministrativeInfoPrefetch = Prefetch(
    "administrative_info",
    queryset=CollectionAdministrativeInfo.objects.prefetch_related("licenses", "rights_holders"),
)


class CollectionRecord(Mapping[str, object]):
    """Immutable mapping storing flattened collection metadata."""
...
class OAIPMHCollectionResult:
    """Container for OAI-PMH ready payloads."""

    def __init__(self, identifier: str, datestamp: str, metadata: CollectionRecord):
        self.identifier = identifier
        self.datestamp = datestamp
        self.metadata = metadata
        self.sets: List[str] = ["collections"]
...
def _build_metadata(collection: Collection) -> CollectionRecord:
    data: MutableMapping[str, object] = {}

    general = collection.general_info.first()
    publication = collection.publication_info.first()
    administrative = collection.administrative_info.first()
    ...
    data["CollectionID"] = collection.identifier
