...
def test_get_or_create_behavior(real_cmd_data):
    ...
    # First import
    header1 = import_collection_header(real_cmd_data)
    
    # Second import with same self link should get existing record
    header2 = import_collection_header(real_cmd_data)
    
    # Should be the same record
    assert header1.pk == header2.pk
    
    # Count should still be 1
    count = CollectionHeader.objects.filter(md_self_link="hdl:11341/0000-0000-0000-3D7C").count()
    assert count == 1
...
@pytest.mark.django_db
def test_multiple_creators_handling():
    ...
    header_model = import_collection_header(cmd)
