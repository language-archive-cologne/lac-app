...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if identifier:
            ...
            if update_bundles:
                self._reindex_bundles_for_collection(
                    collection,
                    bucket_to_use,
                    dry_run=dry_run,
                )
        ...
        if prefix:
            ...
            for collection_key in collection_keys:
                self._reindex_collection(bucket, collection_key, dry_run=dry_run)
                if update_bundles:
                    self._reindex_bundle_keys(bucket, bundle_keys, dry_run=dry_run)
            return 0
