...
@pytest.mark.django_db
def test_legacy_person_rule_without_agent_class_still_matches_user():
    collection = _create_collection()
    bundle = _create_bundle(collection)
    _store_acl(bundle, [{"agent": "alice@uni-koeln.de", "mode": ["acl:Read"]}])
    ...
    user = get_user_model().objects.create_user(
        username="alice@uni-koeln.de",
        password="pass",
        saml_persistent_id="persistent-id-legacy-1",
        acl_agent_uri="urn:lacos:eppn:alice@uni-koeln.de",
    )
    result = service.evaluate(user, bundle)
...
@pytest.mark.django_db
def test_legacy_group_rule_without_agent_class_still_matches_group_member():
    collection = _create_collection()
    bundle = _create_bundle(collection)
    _store_acl(bundle, [{"agent": "urn:lacos:agent:researchers", "mode": ["acl:Read"]}])
    ...
    user = get_user_model().objects.create_user(username="groupie-legacy", password="pass")
    user.groups.add(group)
