...
def test_generate_presigned_post(mock_upload_service):
    ...
    assert "presigned_post" in result, "Result should include presigned_post data"
    assert "url" in result["presigned_post"], "Result should include the presigned URL"
    assert "fields" in result["presigned_post"], "Result should include form fields"
    assert "expires_in" in result, "Result should include expiration time"
    
    # Check the URL format
    assert result["presigned_post"]["url"].startswith("http://"), "URL should be properly formatted"
    
    # Check that the fields contain the necessary S3 form fields
